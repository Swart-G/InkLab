...
import dev.swart.inklab.ui.theme.LocalInkPalette
import dev.swart.inklab.ui.theme.inkDisplayColor
import dev.swart.inklab.ui.theme.paperDisplayColor
...
fun EditorCanvas(vm: EditorViewModel, modifier: Modifier = Modifier) {
    ...
    val controller = remember(vm, vm.currentBoardId) {
        CanvasInputController(vm, android.view.ViewConfiguration.get(context).scaledTouchSlop.toFloat())
    }
        ...
        modifier = modifier
            ...
            .motionEventSpy { event ->
                when (event.actionMasked) {
                    ...
                    MotionEvent.ACTION_HOVER_EXIT -> controller.hover(event)

                    MotionEvent.ACTION_BUTTON_PRESS,
                    MotionEvent.ACTION_BUTTON_RELEASE -> controller.event(event)
                }
            }
            ...
            withTransform({
                ...
                if (notebook) {
                    drawRect(
                        ...
                        Offset(worldLeft - 2f / scale, worldTop + 3f / scale),
                        androidx.compose.ui.geometry.Size(page.width + 4f / scale, page.height + 2f / scale)
                    )
                }
                clipRect(worldLeft, worldTop, worldRight, worldBottom) {
                    ...
                    (if (active) vm.convertedObjects else page.convertedObjects).forEach { item ->
                        if (item.kind == ConvertedInkKind.TEXT) {
                            drawIntoCanvas { canvas ->
                                ...
                                val layout = android.text.StaticLayout.Builder
                                    .obtain(item.content, 0, item.content.length, paint, item.width.toInt().coerceAtLeast(1))
                                    .setIncludePad(false)
                        ...
                        if (active && item.id == vm.selectedConvertedId) {
                            ...
                            drawRoundRect(
                                ...
                                style = Stroke(
                                    1.7f / scale,
                                    pathEffect = PathEffect.dashPathEffect(floatArrayOf(7f / scale, 5f / scale))
                                )
                            )
                        }
                    }
                    ...
                    if (active && vm.lassoPoints.size > 1) {
                        ...
                        drawPath(
                            ...
                            style = Stroke(
                                2.2f / scale,
                                cap = StrokeCap.Round,
                                pathEffect = PathEffect.dashPathEffect(floatArrayOf(8f / scale, 6f / scale))
                            )
                        )
                    }
                    ...
                    if (active) vm.eraserCursor?.let { cursor ->
                        drawCircle(
                            ...
                            vm.inputPreferences.eraserRadius / scale,
                            cursor
                        )
                        drawCircle(
